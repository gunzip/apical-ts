...
describe("Fetch Error Handling", () => {
  it("should generate inner try/catch around fetch request", () => {
    ...
    const result = generateOperationFunction(pathKey, method, operation, [], doc);

    /* Verify inner try/catch structure is present */
    expect(result.functionCode).toContain("/* Inner try/catch for fetch-specific errors */");
    expect(result.functionCode).toContain("let response: Response;");
    expect(result.functionCode).toContain("let data: unknown;");
    expect(result.functionCode).toContain("let minimalResponse: { status: number; headers: Map<string, string> };");

    /* Verify fetch is wrapped in inner try/catch */
    expect(result.functionCode).toContain("try {");
    expect(result.functionCode).toContain("response = await config.fetch(url.toString(), {");
    expect(result.functionCode).toContain("data = await parseResponseBody(response);");
  ...
  it("should preserve existing error handling for non-fetch errors", () => {
    ...
    const unexpectedErrorIndex = result.functionCode.indexOf('kind: "unexpected-error"');
